...
playSynthPattern patternSet mode = do
  ...
  where
    ...
    playPatternWithSynth (midiNote:rest) = do
      ...
      startSynth 
        freq 0.5 Sine       -- Osc1 using the pattern frequency
        (freq/2) 0.3 Square -- Osc2 one octave below
        5.0 Triangle        -- LFO
        0.7 0.3             -- Mixer
        1000 0.3 True       -- Filter
        0.2 1.0             -- Reverb
        0.8                 -- Main volume
...
playSynthVariations patternSet = do
  ...
  where
    ...
    playVaryingSynth (midiNote:rest) index = do
      ...
      startSynth 
        freq 0.6 osc1Type           -- Osc1 using the pattern frequency
        (freq * 1.01) 0.4 osc2Type  -- Osc2 slightly detuned
        (0.5 + fromIntegral index * 0.2) Triangle  -- LFO speed varies
        0.6 0.4                     -- Mixer
        filterFreq 0.4 True         -- Filter freq varies with note
        (0.1 + fromIntegral (index `mod` 5) * 0.1) 2.0  -- Varying reverb
        0.8                         -- Main volume
      
      -- Print the current settings
      putStrLn $ "Note: " ++ show midiNote 
               ++ " | Osc1: " ++ show osc1Type 
               ++ " | Osc2: " ++ show osc2Type
               ++ " | Filter: " ++ show filterFreq ++ " Hz"
